...
class VectorDatabase():
    ...
    def db_check(self):
        ...
        test_vec_yeast = self.database.query(self.database.name ==
                                             'pXP116').get()
        test_vec_gateway = self.database.query(self.database.name ==
                                               'pDONR201').get()
        test_vec_unspecified = self.database.query(self.database.name ==
                                                   'pRS303').get()

        if test_vec_yeast is None:
            self.dl_vectors('Yeast')
        if test_vec_gateway is None:
            self.dl_vectors('Other')
        if test_vec_unspecified is None:
            self.dl_vectors('Unspecified')

    def dl_vectors(self, dburl):

        dburl = dburl.lower()
        dburl = dburl.title()
        dburl = ("http://www.addgene.org/vector-database/query/?q_vdb=*"
                 "+***%20vector_type:" + dburl)
            ...
            try:
                if list[n + 8] == 'yes':
                    if 'vector' in list[n + 7]:
                        ...
                        vec_dict[list[n + 1]] = ("/browse/sequence/%s/"
                                                 "giraffe-analyze_vdb/"
                                                 % r.group()
                                                 )

                    else:
                        vec_dict[list[n + 1]] = ("%ssequences/" %
                                                 str(list[n + 7]))
            except IndexError:
                break
        ...
        for vector in vec_dict:
            if 'giraffe' in vec_dict[vector]:
                try:
                    http = urllib2.urlopen('http://www.addgene.org/'
                                           + vec_dict[vector]).read()
                    giraffelink = re.search(r'src="(/g.*?)"',
                                            http, re.DOTALL).group(1)
                    http = urllib2.urlopen('http://www.addgene.org'
                                           + str(giraffelink)).read()
...
class SitesDatabase():
    ...
    def db_check(self):
        test_att_site = self.database.query(self.database.name
                                            == 'attB4').get()
    ...
    def dl_sites(self):
        ...
        dburl = ("http://chien.neuro.utah.edu/tol2kitwiki/"
                 "index.php/Att_site_sequences"
                 )
        http = urllib2.urlopen(dburl).read()

        src = re.findall(r'gt;.*?([\w\d]+).*?([A-Z]{3,}.*?)[&<]',
                         http, flags=re.DOTALL)
        for n in range(0, len(src), 1):
            if '_' in src[n][0]:
                continue
            else:
                self.database(name=src[n][0], sequence=src[n][1]).put()
    ...
    def dl_enzymes(self):
        ...
        for n in range(0, len(src), 2):
            try:
                ...
                if name.group(1) in enzlist:
                    continue
                else:
                    enzlist.append(str(name.group(1)))
                    self.database(name=str(name.group(1)),
                                  frontsite=str(enzyme.group(1)),
                                  backsite=str(enzyme.group(2))).put()
